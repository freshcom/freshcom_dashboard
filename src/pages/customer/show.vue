      ...
      <div class="data">
        ...
        <div class="block">
          <div class="block-body full">
            <el-table :data="record.orders" class="block-table" :show-header="false" style="width: 100%">
              <el-table-column>
                <template slot-scope="scope">
                  <router-link :to="{ name: 'ShowOrder', params: { id: scope.row.id } }">
                    <span v-if="scope.row.code">[{{scope.row.code}}]</span>
                    <span v-if="!scope.row.code">{{scope.row.grandTotalCents | dollar}}</span>
                    <el-tag v-if="scope.row.status === 'opened'" size="mini">
                      {{$t(`attributes.order.status.${scope.row.status}`)}}
                    </el-tag>
                    <el-tag v-if="scope.row.status != 'opened'" type="gray" size="mini">
                      {{$t(`attributes.order.status.${scope.row.status}`)}}
                    </el-tag>
                  </router-link>
                </template>
              ...
              <el-table-column width="200">
                <template slot-scope="scope">
                  {{scope.row.insertedAt | moment("MMM DD YYYY HH:mm:ss")}}
                </template>
              </el-table-column>
            </el-table>
          </div>
...
export default {
  ...
  mixins: [ShowPage({ storeNamespace: 'customer', name: 'Customer', include: 'orders,unlocks.unlockable' })],
  computed: {
    avatarUrl () {
