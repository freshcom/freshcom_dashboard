...
export default {
  namespaced: true,
  state: {
    record: Account.objectWithDefaults(),
    recordDraft: Account.objectWithDefaults()
  },
  actions: {
    ...
    loadRecord ({ state, commit, rootState }, actionPayload) {
      ...
      return AccountAPI.getRecord(actionPayload.id, options).then(response => {
    ...
    createRecord (context, recordDraft) {
      ...
      return AccountAPI.createRecord(apiPayload).then(response => {
    ...
    deleteRecord ({ commit }, id) {
      return AccountAPI.deleteRecord(id).then(response => {
        commit(MT.RESET_RECORD)
  ...
  mutations: {
    [MT.RESET_RECORD] (state) {
      state.record = Account.objectWithDefaults()
      state.recordDraft = Account.objectWithDefaults()
    },
